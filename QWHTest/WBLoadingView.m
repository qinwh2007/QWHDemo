...
{
    WBLoadingView *loadingView = [self loadViewFromNib];
...
{
    WBLoadingView *loadingView = [self loadingForView:view];
    if (loadingView) {
        [loadingView hide];
    }
}
...
+ (void)hideAllLoadingForView:(UIView *)view
{
    ...
    for (UIView *subview in reverseSubviews) {
        if ([subview isKindOfClass:self]) {
            [(WBLoadingView *)subview hideNoAnimation];
        }
    }
}
