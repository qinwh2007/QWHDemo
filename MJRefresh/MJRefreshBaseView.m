...
- (NSArray *)imagesArr
{
    if (!_imagesArr) {
        _imagesArr = [NSArray arrayWithObjects:
                                                  [UIImage imageNamed:@"小熊斜侧跑0001"],
                                                  [UIImage imageNamed:@"小熊斜侧跑0003"],
                                                  [UIImage imageNamed:@"小熊斜侧跑0005"],
                                                  [UIImage imageNamed:@"小熊斜侧跑0007"],
                                                  [UIImage imageNamed:@"小熊斜侧跑0009"],
                                                  [UIImage imageNamed:@"小熊斜侧跑00011"],
                                                  [UIImage imageNamed:@"小熊斜侧跑00013"],
                                                  [UIImage imageNamed:@"小熊斜侧跑00015"],
                                                  nil];
    }
    return _imagesArr;
}
...
{
    if (!_arrowImage) {
        UIImageView *arrowImage = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 28, 50)];
        arrowImage.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
        if (self.imagesArr) {
            arrowImage.animationImages = _imagesArr;
            arrowImage.animationDuration = 0.7;
        }
        arrowImage.image = _imagesArr.firstObject;
...
{
    if (!_activityView) {
        if (_refreshViewType != MJRefreshViewTypeHeader) {
...
- (void)layoutSubviews
{
    ...
    self.arrowImage.center = CGPointMake(arrowX, -25);
    ...
    switch (state) {
		case MJRefreshStateNormal: // 普通状态
        {
            if (oldState == MJRefreshStateRefreshing) {
                ...
                if (_refreshViewType == MJRefreshViewTypeHeader && _imagesArr) {
