...
{
    frame.size.height = MJRefreshFooterViewHeight;
    if (self = [super initWithFrame:frame]) {
        ...
        self.noMoreDataText = MJRefreshFooterNoMoreDataText;
        self.refreshViewType = MJRefreshViewTypeFooter;
    }
    return self;
}
...
- (void)layoutSubviews
{
    ...
    self.arrowImage.frame = CGRectMake(0, 0, 0, 0);
    self.arrowImage.center = CGPointMake(self.mj_width * 0.5 - 80, self.mj_height/2);
//    self.arrowImage.image = [UIImage imageNamed:MJRefreshSrcName(@"arrow@2x.png")];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if ([MJRefreshContentSize isEqualToString:keyPath]) {
        ...
        // 如果正在刷新，直接返回
        if (self.state == MJRefreshStateRefreshing || self.endingRefresh || self.state == MJRefreshStateNoMoreData)
            return;
