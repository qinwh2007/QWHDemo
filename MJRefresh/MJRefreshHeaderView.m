...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        self.refreshViewType = MJRefreshViewTypeHeader;
...
- (void)adjustStateWithContentOffset
{
    ...
    CGFloat offset = currentOffsetY - happenOffsetY;

    if (offset > -MJRefreshViewHeight && offset < 0) {
        CGFloat num = -offset/2;

        self.arrowImage.center = CGPointMake(self.mj_width * 0.5 - 70, num + 7);
    }
	...
	switch (state) {
		case MJRefreshStateNormal: // 下拉可以刷新
        {
            ...
            if (MJRefreshStateRefreshing == oldState) {
                self.arrowImage.image = self.imagesArr.firstObject;
//                self.arrowImage.transform = CGAffineTransformIdentity;
                ...
                [UIView animateWithDuration:MJRefreshSlowAnimationDuration animations:^{
                    ...
                    self.arrowImage.center = CGPointMake(self.mj_width * 0.5 - 70, -25);
                }];
            } else {
                // 执行动画
		...
		case MJRefreshStatePulling: // 松开可立即刷新
        {
            // 执行动画
            [UIView animateWithDuration:MJRefreshFastAnimationDuration animations:^{
                self.arrowImage.image = self.imagesArr[1];
            }];
			break;
        }
        ...
        {
            self.arrowImage.center = CGPointMake(self.mj_width * 0.5 - 70,  self.mj_height/2 + 7);
            // 执行动画
            [UIView animateWithDuration:MJRefreshFastAnimationDuration animations:^{
                // 1.增加滚动区域
